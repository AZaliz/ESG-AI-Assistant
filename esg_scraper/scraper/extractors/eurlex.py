...
HARDCODED = [
    ...
    },
    {
        ...
        "priority": PRIORITY_PRIMARY,
        # 75pp of legal prose, NO real tables. pdfplumber's "lines" detector
        # misreads the Official Journal two-column layout as table structure,
        # shredding phrases (e.g. "human ri"/"ghts due diligence"). Parse the
        # CSDDD text-only. (ESRS/GHG/IEA have real tables — leave them alone.)
        "force_narrative_only": True,
        "notes": (
...
# Source of truth for the parser's narrative_only flag: any HARDCODED entry
# tagged force_narrative_only is parsed text-only. build_index imports this
# set and matches against the manifest's `url` column (no re-scrape needed).
NARRATIVE_ONLY_URLS = frozenset(
    h["url"] for h in HARDCODED if h.get("force_narrative_only")
)

# Keys that are scraper/parser hints, not Document dataclass fields.
_NON_DOCUMENT_KEYS = ("force_narrative_only",)
...
class EURLexScraper(BaseScraper):
    ...
    def discover(self) -> list:
        docs = []
        for h in HARDCODED:
            fields = {k: v for k, v in h.items() if k not in _NON_DOCUMENT_KEYS}
